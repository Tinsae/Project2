...
function onWindowLoad(){
    Visualforce.remoting.Manager.invokeAction(
        document.getElementById('getDoctorInfo').value, doctor.id,function(result){
            doctor.startTime = result[0] /3600000;
            doctor.endTime = result[1] / 3600000;
            doctor.name = result[2];
            doctor.address = result[3];
            doctor.daysOpen = result[4];
            Visualforce.remoting.Manager.invokeAction(
                document.getElementById('getAppointmentInfo').value, doctor.id, patient.id, function(result){
                    doctor.existingAppointments = result;
                    patient.name = result[result.length-1];
                    createCalendar();
            });
        }
    );
...
function createCalendar(){
    ...
    monthYear.innerHTML = '<button onClick="previous()" id="previous">&lt</button>    '
                                + monthInfo[dateInfo.currentMonth][0] + ' ' + dateInfo.currentYear +
...
function createButtons(currentDiv, year, month, day){
    ...
    var timeString;
    for(var i=doctor.startTime;i<doctor.endTime;i++){
